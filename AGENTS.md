...
- The bundle has two tiers (minimal `~/.claude/` config vs full
  wiki+cron). Don't blur the boundary — the Tier 2 payload that ships
  into `~/.claude/` lives under `home-claude/wiki/` and
  `home-claude/cron/`, not at the root. Tier 2 also covers two
  components that stay outside `home-claude/` by design:
  `scripts/claude-switch.ps1` and `codex/AGENTS.md`.
...
| Automated secret-guard (pre-commit) | [`.githooks/pre-commit`](.githooks/pre-commit) — activate: [`scripts/enable-guard.sh`](scripts/enable-guard.sh) / `.ps1` |
| What changes when adding X also requires touching Y | [`CLAUDE.md`](CLAUDE.md) § Cross-link table |
| Install / verify / bootstrap scripts | [`scripts/`](scripts/) — `install.ps1`, `install-lite.sh`, `self-test.ps1`, `bootstrap-registry.ps1` (fills the `registry.yaml` placeholders), `gen-scheduler.py` (systemd/launchd units from `registry.yaml`) |
