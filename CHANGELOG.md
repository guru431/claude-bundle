...
- **`wiki-compile-kb.py` read the vault strictly.** One page with a non-UTF-8
  byte raised `UnicodeDecodeError` before the first article was touched, so the
  whole compile died and nothing was marked processed. Reads now use
  `errors="replace"`, matching `wiki-compile-sessions.py`.
- **A source article in the system ANSI code page killed the same run.**
  `compile_article()` caught `OSError` only; it now also catches
  `UnicodeDecodeError` and returns `None`, so one unreadable article is one
  failed article, retried next run.
- **The two compilers stripped LLM frontmatter differently.** The CRLF-tolerant
  version existed only in `compile-kb`; `compile-sessions` still required LF and
  duplicated the model's YAML into the page body. Both now call
  `utils.strip_leading_frontmatter()`.

### Fixed — the PDF pair could be regenerated by two different converters

- **`hooks/md2pdf-on-edit.py` hardcoded `~/.claude/bin/md2pdf.py`** while the
  nightly `cron/md2pdf-sync.py` had already learned to resolve it relative to
  the bundle. On a split install (`-PipelineRoot`) the two disagreed. The hook
  now tries `$CLAUDE_MD2PDF`, then its own tree, then `~/.claude/bin`.

### Fixed — Task Scheduler syncer

- **Calendar triggers were registered with today's date.** Syncing at 14:00 gave
  a "Daily 02:30" task a `StartBoundary` in the past, and with
  `StartWhenAvailable=true` the scheduler could fire the nightly job immediately.
  A time that has already passed now starts tomorrow.
- **Launcher/script paths bypassed the argument quoting.** `script_args` went
  through `Quote-Arg`, but the paths were spliced in raw, so a path containing a
  double quote produced an unbalanced command line. Both now go through
  `Quote-Path` (always-quoting, so ordinary paths emit byte-identical arguments
  and no task is re-registered for a quoting change).

### Fixed — shell and PowerShell portability

- **`claude-task-monitor.sh` built its alert with an unquoted heredoc.** The
  body carries task names and descriptions straight from Task Scheduler, and an
  unquoted `<<EOF` performs command substitution on them. Replaced with
  `printf`.
- **The same script used `du --exclude`,** a GNU extension: on BSD/macOS `du`
  the call fails, the size stays empty and the wiki size check silently stops
  running. It is now computed as `wiki` minus `wiki/.git`.
- **`github-push.sh` printed blocked paths unquoted,** so a path with spaces was
  word-split into several bogus "files" in the block report.
- **`self-test.ps1` piped native stderr under `$ErrorActionPreference='Stop'`.**
  In PS 5.1 that is a terminating `NativeCommandError` — the steps whose job is
  to report a failing script would instead kill the self-test. All child
  invocations now go through one `Invoke-Checked` helper.

### Changed — docs

- `AGENTS.md` no longer contradicts `CLAUDE.md` about where the project is
  hosted, distinguishes the shipped `codex/AGENTS.md` mirror from the installed
  `~/.codex/AGENTS.md`, and points at `scripts/` and
